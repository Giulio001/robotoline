...
const ITEM_INFO = {
  ...
  gold: ['Oro', '#d3a836', ''], snow: ['Neve', '#dbe8e8', ''], torch: ['Torcia', '#d88f35', '♨'], bread: ['Pane', '#b97835', '◒'],
...
const PLACEABLE = new Set(['grass', 'dirt', 'stone', 'sand', 'wood', 'leaves', 'planks', 'brick', 'obsidian', 'crystal', 'coal', 'iron', 'gold', 'snow', 'torch']);
const MOB_LABELS = { slime: 'Melma delle radici', boar: 'Cinghiale roccioso', golem: 'Golem antico', wraith: 'Spettro del crepuscolo' };

let scene, camera, renderer, controls, clock, sun, hemiLight, stars;
let worldGroup, entityGroup, dragonGroup, selectionBox;
...
let selectedSlot = 0;
let hotbarItems = ['grass', 'dirt', 'wood', 'stone', 'bread', null, null, null, null];
...
function generatedBlock(x, y, z) {
  if (Math.abs(x) > WORLD_RADIUS || Math.abs(z) > WORLD_RADIUS || y < 0 || y > 22) return null;
...
function isSolid(type) { return Boolean(type && type !== 'water' && type !== 'torch'); }
...
function buildMaterials() {
  const palettes = {
    ...
    gold: ['#d1a632', '#a9801f', '#f1ce55'], snow: ['#d9e7e8', '#c1d6d8', '#effafa'], torch: ['#db8a2c', '#83471d', '#ffc95c']
  };
  const materials = {};
  ...
  materials.water = new THREE.MeshPhongMaterial({ color: 0x3d8da8, transparent: true, opacity: 0.62, shininess: 90, depthWrite: false });
  return materials;
}
...
function initThree() {
  ...
  scene.fog = new THREE.FogExp2(0x92bdc7, 0.018);
  camera = new THREE.PerspectiveCamera(72, innerWidth / innerHeight, 0.05, 220);
...
function buildWorld() {
  ...
  for (let x = -WORLD_RADIUS; x <= WORLD_RADIUS; x++) {
    for (let z = -WORLD_RADIUS; z <= WORLD_RADIUS; z++) {
      const top = Math.max(terrainHeight(x, z) + 7, WATER_LEVEL + 1);
      for (let y = 0; y <= top; y++) {
        ...
        if (exposed) positionsByType[type].push({ x, y, z });
      }
    }
  }
  ...
  for (const [type, positions] of Object.entries(positionsByType)) {
    ...
    const geometry = new THREE.BoxGeometry(type === 'water' ? 1 : 1.001, type === 'water' ? .82 : 1.001, type === 'water' ? 1 : 1.001);
    const mesh = new THREE.InstancedMesh(geometry, blockMaterials[type], positions.length);
    positions.forEach((position, index) => {
      matrix.makeTranslation(position.x, position.y + (type === 'water' ? -.09 : 0), position.z); mesh.setMatrixAt(index, matrix);
    });
    mesh.instanceMatrix.needsUpdate = true; mesh.userData.positions = positions; mesh.userData.blockType = type;
...
function scheduleWorldRebuild() {
  ...
  rebuildTimer = setTimeout(buildWorld, 90);
}
...
function createPlayerModel(player) {
  ...
  group.userData = { entityType: 'player', id: player.id, target: new THREE.Vector3(player.x, player.y - EYE_HEIGHT, player.z), leftArm, rightArm, leftLeg, rightLeg };
...
function refreshProfile(next) {
  ...
  fillHotbar();renderInventory();renderQuests();renderCrafting();renderShop();renderClans();
...
function selectSlot(index){selectedSlot=Math.max(0,Math.min(8,index));fillHotbar()}
function renderInventory(){if(!profile)return;$('#inventory-grid').innerHTML=Object.entries(profile.inventory).sort((a,b)=>a[0].localeCompare(b[0])).map(([item,count])=>`<button class="inventory-item" data-item="${item}"><em>${count}</em>${iconMarkup(item)}<b>${itemName(item)}</b></button>`).join('')||'<p>Lo zaino è vuoto.</p>';$$('.inventory-item').forEach(button=>button.onclick=()=>{const item=button.dataset.item;if(!hotbarItems.includes(item)){hotbarItems[selectedSlot]=item}else selectedSlot=hotbarItems.indexOf(item);fillHotbar();closePanels()})}
...
function openPanel(id){panelOpen=true;controls?.unlock();$('#panel-scrim').classList.remove('hidden');$$('.game-panel').forEach(panel=>panel.classList.toggle('open',panel.id===id));$('#pause-overlay').classList.add('hidden')}
...
function updateTarget() {
  ...
  const blockHits=raycaster.intersectObjects(blockMeshes,false);const entityHits=raycaster.intersectObjects([...mobs.values(),...dragons.values()],true);
  ...
  if(entityHit&&(!blockHit||entityHit.distance<blockHit.distance)){
    const root=findEntityRoot(entityHit.object);currentTarget={kind:root.userData.entityType,id:root.userData.id,distance:entityHit.distance};selectionBox.visible=false;$('#target-label').textContent=root.userData.entityType==='mob'?MOB_LABELS[root.userData.kind]:'Drago cavalcabile';return;
  }
...
function onMouseAction(event) {
  ...
  if(event.button===0&&currentTarget){
    if(currentTarget.kind==='mob'){socket.emit('attack',{id:currentTarget.id,held:selectedItem()});swingEffect()}
    else if(currentTarget.kind==='block'){socket.emit('mine',{...currentTarget.position,block:currentTarget.type});burst(currentTarget.position,ITEM_INFO[currentTarget.type]?.[1]||'#888')}
  } else if(event.button===2&&currentTarget?.kind==='block'){
    const type=selectedItem();if(!PLACEABLE.has(type)){toast('Seleziona un materiale da costruzione.','danger');return}
...
function swingEffect(){camera.rotation.z=-.018;setTimeout(()=>camera.rotation.z=0,100)}
const particles=[];
...
function animateEntities(dt,elapsed){
  ...
  $('#interaction-hint').classList.toggle('hidden',!nearbyDragon&&!mountedDragon);if(nearbyDragon||mountedDragon)$('#interaction-hint span').textContent=mountedDragon?'Scendi dal drago':`Cavalca ${dragons.get(nearbyDragon)?.children.find(c=>c.isSprite)?'il drago':'il drago'}`;
...
function animate(now=0) {
  requestAnimationFrame(animate);if(!scene)return;const dt=Math.min(clock.getDelta(),.05),elapsed=clock.elapsedTime;updateMovement(dt);updateTarget();animateEntities(dt,elapsed);updateSky();emitMovement(now);renderer.render(scene,camera);
}
...
function startGame(data) {
  self=data.self;spawn=data.spawn;worldDay=data.worldDay;overrides=data.blocks||{};profile={...data.profile,quests:data.profile.quests||[]};recipes=data.recipes;shop=data.shop;clans=data.clans||[];
  ...
  initThree();setTimeout(()=>{buildWorld();syncPlayers(data.players);syncMobs(data.monsters);syncDragons(data.dragons);refreshProfile(profile);$('#loading-progress').style.width='100%';setTimeout(()=>{$('#loading-screen').classList.remove('active');$('#hud').classList.remove('hidden');gameStarted=true;controls.lock();toast('Benvenuto a Terranovaland. La piazza è sicura.','quest')},350)},60);animate();
}

$('#login-form').addEventListener('submit',event=>{event.preventDefault();const name=$('#player-name').value.trim();$('#login-error').textContent='';if(name.length<2){$('#login-error').textContent='Inserisci almeno 2 caratteri.';return}localStorage.setItem('terranovaland-name',name);socket.emit('login',{name})});
...
$('#resume-button').onclick=()=>controls.lock();$('#help-button').onclick=()=>{controls.unlock();$('#help-overlay').classList.remove('hidden');$('#pause-overlay').classList.add('hidden')};$('.modal-x').onclick=()=>{$('#help-overlay').classList.add('hidden');controls.lock()};$('#respawn-button').onclick=()=>socket.emit('respawn');
...
addEventListener('keydown',event=>{
  if(event.code==='KeyT'&&gameStarted&&!dead){event.preventDefault();controls.unlock();$('#pause-overlay').classList.add('hidden');$('#chat-form').classList.remove('hidden');$('#chat-input').focus();return}
  if(document.activeElement?.tagName==='INPUT')return;
  if(/^Digit[1-9]$/.test(event.code)){selectSlot(Number(event.code.slice(-1))-1);return}
  if(event.code==='KeyI')openPanel('inventory-panel');else if(event.code==='KeyC')openPanel('craft-panel');else if(event.code==='KeyM')openPanel('shop-panel');else if(event.code==='KeyL')openPanel('clan-panel');else if(event.code==='KeyF')socket.emit('consume','bread');else if(event.code==='KeyE'&&(mountedDragon||nearbyDragon))socket.emit('mountDragon',mountedDragon||nearbyDragon);else keys.add(event.code);
});
...
socket.on('toast',data=>toast(data.text,data.type));
socket.on('blockChanged',data=>{overrides[keyOf(data.x,data.y,data.z)]=data.type;scheduleWorldRebuild()});
socket.on('playerJoined',player=>syncPlayers([...remotePlayers.values()].map(model=>({id:model.userData.id,x:model.position.x,y:model.position.y+EYE_HEIGHT,z:model.position.z,yaw:model.rotation.y,name:'Esploratore'})).concat(player,self)));
...
socket.on('monsterDefeated',data=>{const model=mobs.get(data.id);if(model){burst(model.position,'#e6a34d');entityGroup.remove(model);mobs.delete(data.id)}});
...
socket.on('dragonMounted',data=>{mountedDragon=data.id;if(data.id)toast(`${data.name} ti ha accettato. Ora puoi volare!`,'quest');else toast('Sei sceso dal drago.')});
socket.on('health',health=>{if(profile){profile.health=health;refreshProfile(profile)}});
socket.on('playerDamaged',data=>{const flash=$('#damage-flash');flash.classList.add('active');setTimeout(()=>flash.classList.remove('active'),120);toast(`${MOB_LABELS[data.source]||'Una creatura'} ti ha colpito: −${data.amount}`,'danger')});
socket.on('playerDied',()=>{dead=true;controls.unlock();$('#pause-overlay').classList.add('hidden');$('#death-overlay').classList.remove('hidden')});
